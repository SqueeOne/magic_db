      ...
      {results.length > 0 ? (
        <ul className="w-full py-2 border rounded-lg border-gray-500 bg-white">
          {results.map((result, index) => (
            <Link href={`/single-card/${result.id}`}>
              <li
                key={index}
                className="w-full border-b last:border-b-0 border-gray-300 text-sm text-gray-700 py-3 px-2 flex justify-between hover:bg-blue-300"
              >
                {result.name}
